...
/**
 * Fix - Bithaus format component translator for Reject
...
public class RejectTranslator  {
    ...
    public static cl.bithaus.fix.Reject fromFix(cl.bithaus.qfix.msg.Reject src) 
        throws FieldNotFound {

        cl.bithaus.fix.Reject dest = new cl.bithaus.fix.Reject();

        if(src.isSetRefSeqNum())
            dest.setRefSeqNum(src.getRefSeqNum().getValue());

        if(src.isSetRefTagID())
            dest.setRefTagID(src.getRefTagID().getValue());

        if(src.isSetRefMsgType())
            dest.setRefMsgType(src.getRefMsgType().getValue());

        if(src.isSetSessionRejectReason())
            dest.setSessionRejectReason(src.getSessionRejectReason().getValue());
